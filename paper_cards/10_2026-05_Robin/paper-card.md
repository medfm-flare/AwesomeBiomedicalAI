# Paper Card: A multi-agent system for automating scientific discovery
...
|---|---|---|
| Robin | Author- or catalogue-used system name | Preserve exact capitalization; do not expand beyond the source |
...
| evaluation set | N/A; validated via a lab-in-the-loop experimental workflow on dry age-related macular degeneration (dAMD). | Treat as evaluation evidence, not training data, unless explicitly labelled |
...
- **Official source:** [https://www.nature.com/articles/s41586-026-10652-y](https://www.nature.com/articles/s41586-026-10652-y)
- **Code:** github.com/Future-House/robin
- **Modalities:** text, omics data
- **Source note:** The card is page-grounded to the official PDF.

## 02 One-Sentence Summary

[Analysis] Robin uses Multi-agent LLM system orchestrating Crow (concise literature review), Falcon (deep literature synthesis for candidate evaluation) and Finch (autonomous bioinformatic data analysis) sub-agents to address hypothesis generation, assay selection, candidate proposal, data interpretation Autonomous hypothesis generation, experimental assay selection, therapeutic candidate proposal, interpretation of experimental (RNA-seq) data, and iterative hypothesis refinement; identified and validated ripasudil and KL001 as RPE-phagocytosis-enhancing dAMD candidates and discovered ABCA1 upregulation as a follow-on target.; the evidence is bounded to N/A; validated via a lab-in-the-loop experimental workflow on dry age-related macular degeneration (dAMD). and the evaluation conditions reported in the paper. [Paper: PDF p. 1]
...
- **Surface method:** [Paper] Multi-agent LLM system orchestrating Crow (concise literature review), Falcon (deep literature synthesis for candidate evaluation) and Finch (autonomous bioinformatic data analysis) sub-agents [Paper: PDF pp. 1–2]
- **Training / adaptation:** [Paper] training-free Agents use LLM reasoning and tool use rather than new model training. [Paper: PDF pp. 1–2]
...
**Backbone and tool configuration:** Multi-agent LLM system orchestrating Crow (concise literature review), Falcon (deep literature synthesis for candidate evaluation) and Finch (autonomous bioinformatic data analysis) sub-agents [Paper: PDF pp. 1–2]

![Figure 1 — faithful PDF page view](<figures/page-002.png>)

*Figure 1 is embedded as an unchanged PDF page view. It contributes Architecture and workflow of the Robin system. a, When given the name of a target disease, Robin generates hypotheses and selects the top therapeutic candidates to test experimentally. Robin can autonomously analyse raw data from these experiments to synthesize scientific insights and generate [Paper: PDF p. 2, Figure 1]*
...
- [Paper] **Equation 1:** = 45( ) + 30( ) (1) [Paper: PDF p. 11, Equation 1]

## 10 Experimental Design and Evidence Chain

**Data / population:** N/A; validated via a lab-in-the-loop experimental workflow on dry age-related macular degeneration (dAMD). [Paper: PDF p. 1]

**Downstream tasks:** hypothesis generation, assay selection, candidate proposal, data interpretation Autonomous hypothesis generation, experimental assay selection, therapeutic candidate proposal, interpretation of experimental (RNA-seq) data, and iterative hypothesis refinement; identified and validated ripasudil and KL001 as RPE-phagocytosis-enhancing dAMD candidates and discovered ABCA1 upregulation as a follow-on target. [Paper: PDF p. 1]

**Inventoried tables:**

- **Table 1** — Estimated time-on-task comparison: human versus Robin-assisted discovery [Paper: PDF p. 3, Table 1]
...
| Case study or deployment-style test | Whether the workflow yields a meaningful end product | Domain-specific examples | hypothesis generation, assay selection, candidate proposal, data interpretation Autonomous hypothesis generation, experimental assay selection, therapeutic candidate proposal, interpretation of experimental (RNA-seq) data, and iterative hypothesis refinement; identified and validated ripasudil and KL001 as RPE-phagocytosis-enhancing dAMD candidates and discovered ABCA1 upregulation as a follow-on target. | Feasibility for the reported cases | Prospective reliability across sites or domains | [Paper: main result figures] |

![Figure 2 — faithful PDF page view](<figures/page-004.png>)

*Figure 2 is embedded as an unchanged PDF page view. It contributes Robin generates therapeutic candidate hypotheses for dAMD and analyses experimental data from in vitro experiments. a, Robin proposes several experimental assays and ultimately decides to use an RPE phagocytosis enhancement assay. Robin synthesizes this strategy into an overall goal [Paper: PDF p. 4, Figure 2]*

![Figure 3 — faithful PDF page view](<figures/page-005.png>)

*Figure 3 is embedded as an unchanged PDF page view. It contributes RNA-seq analysis of ARPE-19 cells treated with the ROCK inhibitor Y-27632. a, Robin interprets results from the first experiment and proposes follow-up assays. Text in panel a generated by Robin. b–d, Example plots from a Finch RNA-seq analysis, formatted for readability in publication by a human. [Paper: PDF p. 5, Figure 3]*

![Figure 4 — faithful PDF page view](<figures/page-006.png>)

*Figure 4 is embedded as an unchanged PDF page view. It contributes Ripasudil and KL001 enhance RPE phagocytosis. a, Excerpt of the proposal by Robin for ripasudil. Drawing from the insights from the first round of experimental analysis, Robin proposes ripasudil as a therapeutic candidate for treating dAMD. For result interpretation, a JSON-structured output is [Paper: PDF p. 6, Figure 4]*

## 11 Correct Interpretation of the Conclusions

- **Task scope:** The paper supports the listed tasks, not unrestricted autonomous research or care. [Paper: PDF p. 1]
- **Data boundary:** N/A; validated via a lab-in-the-loop experimental workflow on dry age-related macular degeneration (dAMD). [Paper: PDF p. 1]
- **Model dependence:** Results are conditional on Multi-agent LLM system orchestrating Crow (concise literature review), Falcon (deep literature synthesis for candidate evaluation) and Finch (autonomous bioinformatic data analysis) sub-agents [Paper: PDF pp. 1–2]
