# Paper Card: A brain-inspired agentic architecture to improve planning with LLMs
...
|---|---|---|
| MAP | Author- or catalogue-used system name | Preserve exact capitalization; do not expand beyond the source |
...
| evaluation set | Graph traversal, Tower of Hanoi, PlanBench, and an NLP multi-step reasoning task (eval). | Treat as evaluation evidence, not training data, unless explicitly labelled |

## 01 Basic Information

- **Title:** A brain-inspired agentic architecture to improve planning with LLMs
- **Model / system:** MAP
- **Venue and date:** Nat. Commun. 2025-09
- **Official source:** [https://www.nature.com/articles/s41467-025-63804-5](https://www.nature.com/articles/s41467-025-63804-5)
- **Code:** 
- **Modalities:** text
...
[Analysis] MAP uses Modular Agentic Planner (MAP): brain-inspired modular LLM architecture with separate conflict-monitoring, state-prediction, state-evaluation, task-decomposition and task-coordination modules to address multi-step planning, task decomposition Goal-directed multi-step planning via interaction of specialized brain-inspired modules, addressing LLMs' typical struggles with multi-step reasoning and planning.; the evidence is bounded to Graph traversal, Tower of Hanoi, PlanBench, and an NLP multi-step reasoning task (eval). and the evaluation conditions reported in the paper. [Paper: PDF p. 1]
...
- **Surface method:** [Paper] Modular Agentic Planner (MAP): brain-inspired modular LLM architecture with separate conflict-monitoring, state-prediction, state-evaluation, task-decomposition and task-coordination modules [Paper: PDF pp. 1–2]
- **Training / adaptation:** [Paper] training-free Modules are implemented via prompted LLM calls; no new weights trained. [Paper: PDF pp. 1–2]
...
**Backbone and tool configuration:** Modular Agentic Planner (MAP): brain-inspired modular LLM architecture with separate conflict-monitoring, state-prediction, state-evaluation, task-decomposition and task-coordination modules [Paper: PDF pp. 1–2]

![Figure 1 — faithful PDF page view](<figures/page-002.png>)

*Figure 1 is embedded as an unchanged PDF page view. It contributes Modular agentic planner (MAP). The agent receives states from the environment and high-level goals. These are processed by a set of specialized LLM modules. The Task Decomposer receives the current state and a high-level goal and generates a series of subgoals. The Actor generates proposed actions given a state [Paper: PDF p. 2, Figure 1]*
...
- [Paper] **Equation 3:** Mystery BW ToT 10.4 (16.7) 0.6 (3) [Paper: PDF p. 7, Equation 3]
- [Paper] **Equation 33:** 32 (42) 0 w/o Monitor 27 (33) [Paper: PDF p. 6, Equation 33]
- [Paper] **Equation 42:** 50 (67) 0 w/o Tree Search 32 (42) [Paper: PDF p. 6, Equation 42]
- [Paper] **Equation 67:** 74 (100) 0 w/o Task Decomposer 50 (67) [Paper: PDF p. 6, Equation 67]

## 10 Experimental Design and Evidence Chain

**Data / population:** Graph traversal, Tower of Hanoi, PlanBench, and an NLP multi-step reasoning task (eval). [Paper: PDF p. 1]

**Downstream tasks:** multi-step planning, task decomposition Goal-directed multi-step planning via interaction of specialized brain-inspired modules, addressing LLMs' typical struggles with multi-step reasoning and planning. [Paper: PDF p. 1]

**Inventoried tables:**

- **Table 1** — Ablation study on ToH (3-disk problems) [Paper: PDF p. 6, Table 1]
- **Table 2** — Results on ToH (3-disk problems) using a smaller LLM (Llama3-70B) [Paper: PDF p. 6, Table 2]
- **Table 3** — PlanBench results [Paper: PDF p. 7, Table 3]
- **Table 4** — PlanBench results for a smaller subset of problems (the ﬁrst 30/200 problems for Logistics, and the ﬁrst 100/ 500 problems for Mystery BW) comparing MAP with ToT [Paper: PDF p. 7, Table 4]
- **Table 5** — StrategyQA results [Paper: PDF p. 7, Table 5]
- **Table 6** — Transfer between different planning tasks [Paper: PDF p. 7, Table 6]
...
| Case study or deployment-style test | Whether the workflow yields a meaningful end product | Domain-specific examples | multi-step planning, task decomposition Goal-directed multi-step planning via interaction of specialized brain-inspired modules, addressing LLMs' typical struggles with multi-step reasoning and planning. | Feasibility for the reported cases | Prospective reliability across sites or domains | [Paper: main result figures] |

![Figure 2 — faithful PDF page view](<figures/page-003.png>)

*Figure 2 is embedded as an unchanged PDF page view. It contributes Tower of hanoi task and results. A Depiction of Tower of Hanoi (ToH) task. Original formulation involves disks of different sizes stacked on a set of pegs. Disks must be moved from initial state to goal state while avoiding invalid moves. To test LLMs, an alternative formulation was created involving lists of digits, ensuring that [Paper: PDF p. 3, Figure 2]*

![Figure 3 — faithful PDF page view](<figures/page-005.png>)

*Figure 3 is embedded as an unchanged PDF page view. It contributes Graph traversal tasks and results. A Graph traversal tasks. Steppath: Agent must identify the shortest path from a start state to a goal state. Valuepath: Agent must identify the shortest path from a start state to the state with the largest reward, while avoiding the state with the smaller reward. B Graph traversal results. [Paper: PDF p. 5, Figure 3]*

![Figure 4 — faithful PDF page view](<figures/page-007.png>)

*Figure 4 is embedded as an unchanged PDF page view. It contributes Invalid actions in graph traversal tasks. % invalid' indicates the per­ centage of moves that are invalid (↓ better). GPT-4 Zero-shot, ICL, CoT, and MAD baselines are deterministic, and therefore, a single run was performed on all pro­ blems. Note that MAP did not employ tree search on the Steppath task, and did not [Paper: PDF p. 7, Figure 4]*

## 11 Correct Interpretation of the Conclusions

- **Task scope:** The paper supports the listed tasks, not unrestricted autonomous research or care. [Paper: PDF p. 1]
- **Data boundary:** Graph traversal, Tower of Hanoi, PlanBench, and an NLP multi-step reasoning task (eval). [Paper: PDF p. 1]
- **Model dependence:** Results are conditional on Modular Agentic Planner (MAP): brain-inspired modular LLM architecture with separate conflict-monitoring, state-prediction, state-evaluation, task-decomposition and task-coordination modules [Paper: PDF pp. 1–2]
