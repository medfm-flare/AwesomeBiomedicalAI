# Paper Card: Optimizing genomics-aware clinical agents in precision oncology
...
|---|---|---|
| Genomics-aware Clinical Agent | Author- or catalogue-used system name | Preserve exact capitalization; do not expand beyond the source |
...
- **Title:** Optimizing genomics-aware clinical agents in precision oncology
- **Model / system:** Genomics-aware Clinical Agent
- **Venue and date:** npj Syst. Biol. Appl. 2026-06
- **Official source:** [https://www.nature.com/articles/s41540-026-00753-9](https://www.nature.com/articles/s41540-026-00753-9)
...
[Analysis] Genomics-aware Clinical Agent uses Workflow-orchestrated clinical agent restricted to 20 curated genomics and oncology tools backed by a versioned knowledge stack; evaluated with multiple frontier LLM backbones to address Genomics-aware precision-oncology decision support, including guideline concordance, patient-specific molecular interpretation, recommendation generation and safety-focused reasoning; the evidence is bounded to N/A and the evaluation conditions reported in the paper. [Paper: PDF p. 1]
...
- **Surface method:** [Paper] Workflow-orchestrated clinical agent restricted to 20 curated genomics and oncology tools backed by a versioned knowledge stack; evaluated with multiple frontier LLM backbones [Paper: PDF pp. 1–2]
...
**Backbone and tool configuration:** Workflow-orchestrated clinical agent restricted to 20 curated genomics and oncology tools backed by a versioned knowledge stack; evaluated with multiple frontier LLM backbones [Paper: PDF pp. 1–2]
...
**Downstream tasks:** Genomics-aware precision-oncology decision support, including guideline concordance, patient-specific molecular interpretation, recommendation generation and safety-focused reasoning [Paper: PDF p. 1]
...
| Case study or deployment-style test | Whether the workflow yields a meaningful end product | Domain-specific examples | Genomics-aware precision-oncology decision support, including guideline concordance, patient-specific molecular interpretation, recommendation generation and safety-focused reasoning | Feasibility for the reported cases | Prospective reliability across sites or domains | [Paper: main result figures] |
...
- **Model dependence:** Results are conditional on Workflow-orchestrated clinical agent restricted to 20 curated genomics and oncology tools backed by a versioned knowledge stack; evaluated with multiple frontier LLM backbones [Paper: PDF pp. 1–2]
...
**Prospective guarded evaluation of Genomics-aware Clinical Agent.** [Hypothesis] A calibrated abstention policy plus clinician approval will preserve most useful actions while reducing severe errors. Delta: add risk-tiered action permissions and uncertainty-triggered handoff. Validation: prospective silent deployment on an external site, followed by a preregistered clinician-in-the-loop comparison; primary endpoints are severe-error rate, calibration, time, and resource use. Falsifier: no reduction in severe errors or clinically important delay. Failure modes: workflow adaptation, alert fatigue, distribution shift. Innovation status: unverified; prior-art search required.