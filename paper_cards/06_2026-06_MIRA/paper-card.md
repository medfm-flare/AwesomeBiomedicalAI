# Paper Card: Towards autonomous medical artificial intelligence agents
...
>
> Card completeness: Complete relative to the processed source (30 PDF pages)

## Terminology Ledger

| Canonical term | First-use definition | Decision |
|---|---|---|
| MIRA | Author- or catalogue-used system name | Preserve exact capitalization; do not expand beyond the source |
...
| evaluation set | N/A; evaluated on >500 MIMIC-IV emergency department cases spanning 8 diagnoses. | Treat as evaluation evidence, not training data, unless explicitly labelled |

## 01 Basic Information

- **Title:** Towards autonomous medical artificial intelligence agents
- **Model / system:** MIRA
- **Venue and date:** Nature 2026-06
- **Official source:** [https://www.nature.com/articles/s41586-026-10675-5](https://www.nature.com/articles/s41586-026-10675-5)
- **Code:** github.com/Dyke-F/MIRA
- **Modalities:** text, EHR data
- **Source note:** The card is page-grounded to the official PDF.

## 02 One-Sentence Summary

[Analysis] MIRA uses LLM-based autonomous agent with 11 tools, operating in a sandboxed, FHIR-compliant EHR environment (ICD, LOINC, ATC, NDC, RxNorm, SNOMED-CT coding) to address history-taking, diagnosis, treatment planning, medication prescribing, admission decisions Autonomous EHR-integrated clinical decision-making: history-taking via patient-agent dialogue, ordering/interpreting labs, imaging and microbiology tests, differential diagnosis generation, treatment planning, medication prescribing, procedure scheduling and admission decisions.; the evidence is bounded to N/A; evaluated on >500 MIMIC-IV emergency department cases spanning 8 diagnoses. and the evaluation conditions reported in the paper. [Paper: PDF p. 1]
...
- **Surface method:** [Paper] LLM-based autonomous agent with 11 tools, operating in a sandboxed, FHIR-compliant EHR environment (ICD, LOINC, ATC, NDC, RxNorm, SNOMED-CT coding) [Paper: PDF pp. 1–2]
- **Training / adaptation:** [Paper] training-free No new model trained; MIRA (Medical Intelligence for Reasoning and Action) is an agent framework over an existing LLM backbone. [Paper: PDF pp. 1–2]
...
**Backbone and tool configuration:** LLM-based autonomous agent with 11 tools, operating in a sandboxed, FHIR-compliant EHR environment (ICD, LOINC, ATC, NDC, RxNorm, SNOMED-CT coding) [Paper: PDF pp. 1–2]

![Figure 1 — faithful PDF page view](<figures/page-003.png>)

*Figure 1 is embedded as an unchanged PDF page view. It contributes MIRA workflow. MIRA is an autonomous medical AI agent that operates within an EHR sandbox, using a suite of tools to simulate clinical workflows: it can order tests, synthesize results and produce diagnoses and treatment plans while interacting through chat with a patient AI agent that is grounded [Paper: PDF p. 3, Figure 1]*
...
**Data / population:** N/A; evaluated on >500 MIMIC-IV emergency department cases spanning 8 diagnoses. [Paper: PDF p. 1]

**Downstream tasks:** history-taking, diagnosis, treatment planning, medication prescribing, admission decisions Autonomous EHR-integrated clinical decision-making: history-taking via patient-agent dialogue, ordering/interpreting labs, imaging and microbiology tests, differential diagnosis generation, treatment planning, medication prescribing, procedure scheduling and admission decisions. [Paper: PDF p. 1]
...
| Case study or deployment-style test | Whether the workflow yields a meaningful end product | Domain-specific examples | history-taking, diagnosis, treatment planning, medication prescribing, admission decisions Autonomous EHR-integrated clinical decision-making: history-taking via patient-agent dialogue, ordering/interpreting labs, imaging and microbiology tests, differential diagnosis generation, treatment planning, medication prescribing, procedure scheduling and admission decisions. | Feasibility for the reported cases | Prospective reliability across sites or domains | [Paper: main result figures] |

![Figure 2 — faithful PDF page view](<figures/page-004.png>)

*Figure 2 is embedded as an unchanged PDF page view. It contributes Evaluation of patient agent robustness. a, Bar charts quantifying response stability to rephrased questions and statements, stratified by evaluator (physician reviewer versus an independent LLM as a judge) and source group (MIRA, mixed-experience physicians (physicians), or board- [Paper: PDF p. 4, Figure 2]*

![Figure 3 — faithful PDF page view](<figures/page-005.png>)

*Figure 3 is embedded as an unchanged PDF page view. It contributes Diagnostic accuracy, reasoning and test selection performance of MIRA compared with four board-certified physicians. a, Left, overall diagnostic accuracy across 8 target conditions using MIMIC-IV information as ground truth (n = 574); MIRA showed high accuracy with straightforward [Paper: PDF p. 5, Figure 3]*

![Figure 4 — faithful PDF page view](<figures/page-007.png>)

*Figure 4 is embedded as an unchanged PDF page view. It contributes Disease management and guideline adherence of MIRA. a, Top five procedures requested by MIRA for patient cases with appendicitis and cholecystitis, highlighting exact and equivalent ICD code matches. Only cases where the initial diagnosis was correct are shown, consistent with the reporting [Paper: PDF p. 7, Figure 4]*

![Figure 5 — faithful PDF page view](<figures/page-008.png>)

*Figure 5 is embedded as an unchanged PDF page view. It contributes Safety and robustness evaluation of MIRA. Disease management safety of MIRA was evaluated by a board-certified physician, who was blinded to source and assessed a 50:50 split of responses from MIRA and human physicians. a, Horizontal stacked bars depict case-level counts and percentages [Paper: PDF p. 8, Figure 5]*

## 11 Correct Interpretation of the Conclusions

- **Task scope:** The paper supports the listed tasks, not unrestricted autonomous research or care. [Paper: PDF p. 1]
- **Data boundary:** N/A; evaluated on >500 MIMIC-IV emergency department cases spanning 8 diagnoses. [Paper: PDF p. 1]
- **Model dependence:** Results are conditional on LLM-based autonomous agent with 11 tools, operating in a sandboxed, FHIR-compliant EHR environment (ICD, LOINC, ATC, NDC, RxNorm, SNOMED-CT coding) [Paper: PDF pp. 1–2]
...
**Prospective guarded evaluation of MIRA.** [Hypothesis] A calibrated abstention policy plus clinician approval will preserve most useful actions while reducing severe errors. Delta: add risk-tiered action permissions and uncertainty-triggered handoff. Validation: prospective silent deployment on an external site, followed by a preregistered clinician-in-the-loop comparison; primary endpoints are severe-error rate, calibration, time, and resource use. Falsifier: no reduction in severe errors or clinically important delay. Failure modes: workflow adaptation, alert fatigue, distribution shift. Innovation status: unverified; prior-art search required.