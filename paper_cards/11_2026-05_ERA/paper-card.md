# Paper Card: An AI system to help scientists write expert-level empirical software
...
>
> Card completeness: Complete relative to the processed source (22 PDF pages)

## Terminology Ledger

| Canonical term | First-use definition | Decision |
|---|---|---|
| ERA | Author- or catalogue-used system name | Preserve exact capitalization; do not expand beyond the source |
...
- **Official source:** [https://www.nature.com/articles/s41586-026-10658-6](https://www.nature.com/articles/s41586-026-10658-6)
- **Code:** github.com/google-research/era
- **Modalities:** text, code
- **Source note:** The card is page-grounded to the official PDF.

## 02 One-Sentence Summary

[Analysis] ERA uses LLM agent (ERA, Empirical Research Assistance) to address bioinformatics method discovery, epidemiological forecasting In bioinformatics, ERA discovered 40 novel methods for single-cell data analysis that outperformed the top human-developed methods on a public leaderboard. In epidemiology, ERA generated 14 models that outperformed the CDC ensemble and all other individual models for forecasting COVID-19 hospitalizations.; the evidence is bounded to N/A and the evaluation conditions reported in the paper. [Paper: PDF p. 1]
...
- **Surface method:** [Paper] LLM agent (ERA, Empirical Research Assistance) [Paper: PDF pp. 1–2]
- **Training / adaptation:** [Paper] tree search Uses tree search over generated programs rather than additional model training. [Paper: PDF pp. 1–2]
...
**Backbone and tool configuration:** LLM agent (ERA, Empirical Research Assistance) [Paper: PDF pp. 1–2]

![Figure 1 — faithful PDF page view](<figures/page-002.png>)

*Figure 1 is embedded as an unchanged PDF page view. It contributes Schematic and performance of ERA. a, Schematic of the ERA algorithm. A scorable task, together with research ideas proposing methods to solve the task, are fed to a LLM, which produces code to evaluate the scorable task in a sandbox. This is then embedded within a tree search algorithm, in which [Paper: PDF p. 2, Figure 1]*
...
- [Paper] **Equation 1:** PUCT = + × ( ) (1) [Paper: PDF p. 9, Equation 1]
- [Paper] **Equation 3:** Hub based on the following inclusion criteria: (1) the method must be reproducible solely using historical COVID-19 hospitalization data, without reliance on external predictor variables; (2) the model submis- sion must include predictions across all specified time horizons; and (3) [Paper: PDF p. 11, Equation 3]
- [Paper] **Equation 6:** gave a selection of eight models for replication. To instruct the search algorithm, we provided the method descrip- tions from the original authors’ official submission metadata. For example, the metadata for the UMass-ar6_pooled model states: “AR(6) [Paper: PDF p. 11, Equation 6]

## 10 Experimental Design and Evidence Chain

**Data / population:** N/A [Paper: PDF p. 1]

**Downstream tasks:** bioinformatics method discovery, epidemiological forecasting In bioinformatics, ERA discovered 40 novel methods for single-cell data analysis that outperformed the top human-developed methods on a public leaderboard. In epidemiology, ERA generated 14 models that outperformed the CDC ensemble and all other individual models for forecasting COVID-19 hospitalizations. [Paper: PDF p. 1]

**Inventoried tables:**

- **Table 1** — Performance comparison of ERA and best of N on the scRNA-seq batch integration and epidemiological modelling tasks [Paper: PDF p. 6, Table 1]
...
| Case study or deployment-style test | Whether the workflow yields a meaningful end product | Domain-specific examples | bioinformatics method discovery, epidemiological forecasting In bioinformatics, ERA discovered 40 novel methods for single-cell data analysis that outperformed the top human-developed methods on a public leaderboard. In epidemiology, ERA generated 14 models that outperformed the CDC ensemble and all other individual models for forecasting COVID-19 hospitalizations. | Feasibility for the reported cases | Prospective reliability across sites or domains | [Paper: main result figures] |

![Figure 2 — faithful PDF page view](<figures/page-004.png>)

*Figure 2 is embedded as an unchanged PDF page view. It contributes Performance of ERA on scRNA-seq batch integration. a, Schematic of the batch integration task, in which disparate datasets (blue and red) are processed to remove batch effects in the data while retaining biological variability. b, Performance of tree search (method names bolded and suffixed [Paper: PDF p. 4, Figure 2]*

![Figure 3 — faithful PDF page view](<figures/page-005.png>)

*Figure 3 is embedded as an unchanged PDF page view. It contributes Performance of ERA on COVID-19 forecasting. a, Rolling validation window used for the forecasting experiments. The output of each search is validated internally on a preceding block of time (blue) and the resulting model is then used to make predictions for its corresponding forecasting [Paper: PDF p. 5, Figure 3]*
...
- **Model dependence:** Results are conditional on LLM agent (ERA, Empirical Research Assistance) [Paper: PDF pp. 1–2]
