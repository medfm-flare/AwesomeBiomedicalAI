# Paper Card: Towards conversational artificial intelligence for disease management
...
|---|---|---|
| AMIE | Author- or catalogue-used system name | Preserve exact capitalization; do not expand beyond the source |
...
| evaluation set | RxQA, a multiple-choice medication-reasoning benchmark derived from US/UK drug formularies; 100 multi-visit case scenarios aligned with UK NICE Guidance and BMJ Best Practice (eval). | Treat as evaluation evidence, not training data, unless explicitly labelled |
...
- **Official source:** [https://www.nature.com/articles/s41586-026-10764-5](https://www.nature.com/articles/s41586-026-10764-5)
- **Code:** 
- **Modalities:** text
...
[Analysis] AMIE uses LLM-based agentic system built on Gemini's long-context capabilities, combining in-context retrieval with structured reasoning to address disease management reasoning, medication selection Multi-visit clinical management dialogue: investigation selection, medication prescribing, and guideline-aligned reasoning; non-inferior to 21 primary care physicians in a blinded OSCE study.; the evidence is bounded to RxQA, a multiple-choice medication-reasoning benchmark derived from US/UK drug formularies; 100 multi-visit case scenarios aligned with UK NICE Guidance and BMJ Best Practice (eval). and the evaluation conditions reported in the paper. [Paper: PDF p. 1]
...
- **Surface method:** [Paper] LLM-based agentic system built on Gemini's long-context capabilities, combining in-context retrieval with structured reasoning [Paper: PDF pp. 1–2]
- **Training / adaptation:** [Paper] training-free No fine-tuning; grounds reasoning in clinical guidelines and drug formularies via in-context retrieval over the base Gemini model. [Paper: PDF pp. 1–2]
...
**Backbone and tool configuration:** LLM-based agentic system built on Gemini's long-context capabilities, combining in-context retrieval with structured reasoning [Paper: PDF pp. 1–2]

![Figure 1 — faithful PDF page view](<figures/page-002.png>)

*Figure 1 is embedded as an unchanged PDF page view. It contributes Overview of contributions. The capabilities of AMIE were advanced towards management of disease over the course of multiple visits and in concordance with authoritative knowledge in the form of clinical guidelines. AMIE leverages an agentic system incorporating a dialogue agent for [Paper: PDF p. 2, Figure 1]*
...
**Data / population:** RxQA, a multiple-choice medication-reasoning benchmark derived from US/UK drug formularies; 100 multi-visit case scenarios aligned with UK NICE Guidance and BMJ Best Practice (eval). [Paper: PDF p. 1]

**Downstream tasks:** disease management reasoning, medication selection Multi-visit clinical management dialogue: investigation selection, medication prescribing, and guideline-aligned reasoning; non-inferior to 21 primary care physicians in a blinded OSCE study. [Paper: PDF p. 1]
...
| Case study or deployment-style test | Whether the workflow yields a meaningful end product | Domain-specific examples | disease management reasoning, medication selection Multi-visit clinical management dialogue: investigation selection, medication prescribing, and guideline-aligned reasoning; non-inferior to 21 primary care physicians in a blinded OSCE study. | Feasibility for the reported cases | Prospective reliability across sites or domains | [Paper: main result figures] |

![Figure 2 — faithful PDF page view](<figures/page-003.png>)

*Figure 2 is embedded as an unchanged PDF page view. It contributes Reasoning and planning under structural constraints. Inference- time decoding constraints are applied to constrain the model output to a predefined JSON structure and sets of values. The structures are defined in Python code, generated on the basis of the set of retrieved guidelines and [Paper: PDF p. 3, Figure 2]*

![Figure 3 — faithful PDF page view](<figures/page-004.png>)

*Figure 3 is embedded as an unchanged PDF page view. It contributes Overview of randomized study design. A PCP and AMIE performed (in randomized order) 3 virtual remote OSCE visits with 100 simulated patients through online multiturn synchronous text chat, building on an initial patient presentation with subsequent updates on symptoms, treatment responses and [Paper: PDF p. 4, Figure 3]*

![Figure 4 — faithful PDF page view](<figures/page-005.png>)

*Figure 4 is embedded as an unchanged PDF page view. It contributes Management plan quality. The quality of management plans for each of three visits per scenario was measured as the proportion of cases in which AMIE or PCPs received favourable median ratings from a panel of three independent specialist physicians. We present overall quality criteria [Paper: PDF p. 5, Figure 4]*

![Figure 5 — faithful PDF page view](<figures/page-006.png>)

*Figure 5 is embedded as an unchanged PDF page view. It contributes MXEKF performance. Relative performance of PCPs and AMIE on each of the ten MXEKF evaluation axes in terms of preferences expressed by specialist physicians and patient actors, respectively. Preferences were derived from independent ratings (on a five-point scale ranging from ‘Poor’ [Paper: PDF p. 6, Figure 5]*

## 11 Correct Interpretation of the Conclusions

- **Task scope:** The paper supports the listed tasks, not unrestricted autonomous research or care. [Paper: PDF p. 1]
- **Data boundary:** RxQA, a multiple-choice medication-reasoning benchmark derived from US/UK drug formularies; 100 multi-visit case scenarios aligned with UK NICE Guidance and BMJ Best Practice (eval). [Paper: PDF p. 1]
- **Model dependence:** Results are conditional on LLM-based agentic system built on Gemini's long-context capabilities, combining in-context retrieval with structured reasoning [Paper: PDF pp. 1–2]
...
**Prospective guarded evaluation of AMIE.** [Hypothesis] A calibrated abstention policy plus clinician approval will preserve most useful actions while reducing severe errors. Delta: add risk-tiered action permissions and uncertainty-triggered handoff. Validation: prospective silent deployment on an external site, followed by a preregistered clinician-in-the-loop comparison; primary endpoints are severe-error rate, calibration, time, and resource use. Falsifier: no reduction in severe errors or clinically important delay. Failure modes: workflow adaptation, alert fatigue, distribution shift. Innovation status: unverified; prior-art search required.