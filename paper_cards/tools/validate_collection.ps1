...
$resolvedCardsRoot = (Resolve-Path -LiteralPath $CardsRoot).Path
$folders = @(Get-ChildItem -LiteralPath $resolvedCardsRoot -Directory | Where-Object { $_.Name -match '^\d{2}_' } | Sort-Object Name)

$cataloguePath = Join-Path (Split-Path -Parent $resolvedCardsRoot) 'AI_agent.md'
$catalogue = Get-Content -LiteralPath $cataloguePath -Raw
$mapping = Get-Content -LiteralPath (Join-Path $resolvedCardsRoot 'catalogue-map.json') -Raw | ConvertFrom-Json
...
foreach ($folder in $folders) {
    ...
    if (Test-Path -LiteralPath (Join-Path $folder.FullName 'source_bundle.json')) {
        $errors.Add("$($folder.Name): source_bundle.json must not be published")
    }
    foreach ($textFile in Get-ChildItem -LiteralPath $folder.FullName -File | Where-Object { $_.Extension -in @('.md','.json','.txt','.yaml','.yml') }) {
        $textContent = Get-Content -LiteralPath $textFile.FullName -Raw
        if ($textContent -match '[A-Za-z]:\\|file://') {
            $errors.Add("$($folder.Name): $($textFile.Name) contains a local absolute path")
        }
    }
    ...
    $referencedImages = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::OrdinalIgnoreCase)
    foreach ($md in Get-ChildItem -LiteralPath $folder.FullName -File -Filter '*.md') {
        ...
        foreach ($match in [regex]::Matches($text, '!\[[^]]*\]\((?:<)?([^)>]+)(?:>)?\)')) {
            ...
            if ($target -notmatch '^https?://') {
                ...
                try { [void]$referencedImages.Add([IO.Path]::GetFullPath($resolved)) } catch {}
                if (-not (Test-Path -LiteralPath $resolved)) { $errors.Add("$($folder.Name): broken image link $target") }
            }
        }
    }
    $figuresPath = Join-Path $folder.FullName 'figures'
    if (Test-Path -LiteralPath $figuresPath) {
        foreach ($asset in Get-ChildItem -LiteralPath $figuresPath -File) {
            if (-not $referencedImages.Contains($asset.FullName)) {
                $errors.Add("$($folder.Name): orphan figure asset $($asset.Name)")
            }
        }
    }
    ...
    if (Test-Path -LiteralPath $auditPath) {
        ...
        if ([int]$audit.summary.warnings -ne 0) {
            if ($folder.Name -match '_Virtual_Lab$' -and [int]$audit.summary.warnings -eq 1) {
